...
def modelType =  args[0]; // "linear" or not
/*
* SET MODEL TYPES
*
* Options:
...
def knows_file=dataPath + "wiki_knows.txt"
def trusts_file=dataPath + "wiki_trusts.txt"
...
InserterUtils.loadDelimitedDataTruth(inserter, knows_file)

inserter = data.getInserter(trusts, fullTrusts)
InserterUtils.loadDelimitedDataTruth(inserter, trusts_file)
...
for (int fold = 0; fold < folds; fold++) {
	...
	for (int configIndex = 0; configIndex < configs.size(); configIndex++) {
		 ...
		 */
		File outFile = new File("output/wiki/inferred.edges");
